<template>
  ...
  <div style="color: #666">
    ...
    <div class="pd-10" style="font-size: 20px;color:#42b983;cursor: pointer">{{article.name}}</div>
    ...
    <mavon-editor  class="md" :value="article.content" :subfield="false" :defaultOpen="'preview'"
                   :toolbarsFlag="false" :editable="false" :scrollStyle="true"
                   :ishljs="true" />
    </div>
  </div>
        ...
        <div v-if="item.children.length"  style="padding-left: 100px">
          <div v-for="subItem in item.children" :key="subItem.id" style="background-color: #f0f0f0;padding: 10px 20px">
            ...
            <div style="font-size:14px;padding: 5px 0;line-height: 25px">
              ...
              <div style="display: flex;margin-top: 5px;padding-left: 5px">
                <div style="width: 200px">
                  <i class="el-icon-time"></i><span style="margin-left: 5px">{{subItem.time}}</span>
                </div>
                <div style="text-align:right;flex:1">
                  <el-button style="margin-left: 5px" type="text" @click="handle(subItem.id)">回复</el-button>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.request.post("/comment/insert", this.commentForm).then(res => {
        if (res.code === '200') {
          this.$message.success("评论成功")
          this.commentForm={} //清空文本框
          //刷新评论区
          this.loadComment()
