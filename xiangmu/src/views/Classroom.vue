...
<div>
  ...
  <el-table :data="tableData" border stripe  :header-cell-class-name="'headBg'" @selection-change="handleSelectionChange">
    ...
    <el-table-column prop="name" label="教室名称" width="140"></el-table-column>
    <el-table-column prop="capacity" label="教室容量" width="120"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    //是否启用
    changeEnable(row) {
      this.request.post("/classroom/update", row).then(res => {
        if (res.code == "200") {
    ...
    //添加
    save(){
      this.request.post("/classroom/insert",this.form).then(res =>{
        if(res.code=='200'){
    ...
    handleAdd1(){
      ...
      this.form = {}
      //请求图标的数据
      this.request.get("/classroom/icon",{
      }).then(res =>{
        this.options=res.data
