<template>
  <div style="color: #666">
    ...
    <div style="padding: 20px 0;border-bottom:1px dashed #ccc" v-for="item in tableData" :key="item.id">
       <div class="pd-10" style="font-size: 20px;color:#42b983;cursor: pointer" @click="$router.push({path:'/front/articleDetail',query:{id:item.id}})">{{item.name}}</div>
...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/article/insert", this.form).then(res => {
        if (res.code === '200') {
